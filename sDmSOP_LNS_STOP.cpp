...
vector<vector<ll>> cluster;
vector<vector<ld>> extended_cost;
vector<ll> profit;
vector<pair<ld, ll>> proximity;
...
void input(string file_name) {
    ...
    cost.resize(n+1,vector<ld>(n+1, INF));
    profit.resize(r);
...
void calculate_extended_cost() {
    extended_cost.resize(n + r + 1, vector<ld>(n + r + 1, INF));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
...
*/
vector<pair<ld,ll>> find_next_state_of_dp(ll  cluster1, ll  cluster2, vector<pair<ld,ll>> dp_row) {
    // cerr<<"Finding the next dp state for movement from "<<cluster1<<" to "<<cluster2<<"....."<<endl;

    vector<pair<ld,ll>> dp_cur(sz(cities_in_cluster[cluster2]), {INF,0});
...
// Add depot cost here
vd  L(vector<vector<ll>>& u, bool printCity=false) {
    ...
    for(ll  i = 0; i < sz(u)-1; i++) {
        // cout<<"i = "<<i<<endl;
        vector<vector<pair<ld,ll>>> dp(sz(u[i])+2); // Minimum cost of reaching each vertex of each cluster
...
vector<vector<ll>> construct_alternate_initial_solution() {
    ...
    vector<ld> u (m+1), v(r);
    vector<ll> matching (r), way (r);
    for (ll  i=1; i<=m; i++) {
        matching[0] = i;
        ll  j0 = 0;
        vector<ld> minv (r, INF);
        vector<char> used (r, false);
        do {
            used[j0] = true;
            ll  i0 = matching[j0], j1;
            ld delta = INF;
            for (ll  j=1; j < r; j++){
                // cout << "i0 = " << i0 << " j = " << j << endl;
                if (!used[j]) {
                    ld  cur = weight_row[j]-u[i0]-v[j];
 ...
 */
ld cost_of_path(vector<ll> &l) {
    vector<vector<pair<ld,ll>>> dp(sz(l)+2); // Minimum cost of reaching each vertex of each cluster
    //first stores min cost, second stores prev city
    dp[0].pb({0,1});
    ll  cluster1 = 0;
    for(ll  j = 0; j < sz(l); j++) {
...
vector<vector<ll>> remove_2(vector<vector<ll>> u) {
    vector<pair<ld, ii>> cost;
    for (int i = 0; i < sz(u) - 1; i++) {
        ld x = cost_of_path(u[i]);
...
vector<vector<ll>> remove_5(vector<vector<ll>> u) {
    ...
    auto [p, q] = rp_select(cost, 5).second;
    tie(p, q) = minmax(p, q);
...
pair<vector<ld>, vector<ld>> preprocess_costs(vector<vector<ll>> &u) {
    vector<ld> dp(n + 1, INF), rdp(n + 1, INF);
    ...
    for (int i = 0; i < sz(u); i++) {
        ...
        for (int j = sz(u[i]) - 2; j >= 0; j--) {
            for (int k : cities_in_cluster[u[i][j]]) {
                for (int v : cities_in_cluster[u[i][j + 1]]) {
                    dp[k] = min(dp[k], dp[v] + cost[k][v]);
                }
            }
        }
    }
    vector<ld> g(r, INF), h(r, INF);
    for (int i = 0; i < r; i++) {
...
vector<pair<ld, ii>> insertion_costs(vector<vector<ll>> &u, int v) {
    auto [g, h] = preprocess_costs(u);
    vector<pair<ld, ii>> res;
    for (int i = 0; i < sz(u) - 1; i++) {
        ld x = cost_of_path(u[i]);
        for (int j = 0; j <= sz(u[i]); j++) {
            int prev = j > 0 ? u[i][j - 1] : 0, next = j < u[i].size() ? u[i][j] : 0;
            ld bound1 = g[prev] + extended_cost[prev][v] + extended_cost[v][next] + h[next];
            if (bound1 > Tmax / m) {
                continue;
            }
            ld z = g[prev] + h[next];
...
ii find_least_cost_insertion_position(vector<vector<ll>> &u, int v) {
    vector<pair<ld, ii>> l = insertion_costs(u, v);
    if (l.empty()) {
        return {-1, -1};
    }
...
vector<vector<ll>> parallel_insert(vector<vector<ll>> u, function<int(vector<vector<pair<ld, ii>>>)> select) {
    vector<ll> left;
    while (!u[m].empty()) {
        vector<vector<pair<ld, ii>>> costs;
        vector<ll> v;
        vector<ll> temp;
        for (int i : u[m]) {
            vector<pair<ld, ii>> cost = insertion_costs(u, i);
            if (!cost.empty()) {
...
vector<vector<ll>> insert_4(vector<vector<ll>> u) {
    return parallel_insert(u, [](vector<vector<pair<ld, ii>>> v) {
        pair<ld, ll> res = {INF, -1};
...
vector<vector<ll>> mtsp_lns(ll  stoppingTime) {
    ...
    while(1) {
        ...
        ll p_new = P(u11);
        if(!tourInvalid(u11) && no_salesmen_empty(u11) && p_new > p_current) {
            if (p_new > p_max) {
