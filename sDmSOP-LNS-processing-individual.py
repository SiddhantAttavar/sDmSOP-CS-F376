...
ITERATION_COUNT = 5
path = os.path.dirname(os.path.abspath(__file__)) + '/VNS-LNS-Hybrid-Solutions-Individual/' 
...
workbook = xlsxwriter.Workbook('VNS_LNS_Hybrid_profit_individual.xlsx')
...
worksheet.write(0, 5, 'VNS_LNS_Hybrid time (in seconds)')
...
# compile_code = 'g++ sDmSOP_VNS_LNS_hybrid_individual.cpp -g -fsanitize=address,undefined -o sdmi_LNS.out'
compile_code = 'g++ sDmSOP_VNS_LNS_hybrid_individual.cpp -o sdmi_LNS.out'
		...
		for t in travellers:
			for g in g_values:
				for w in w_values:
					...
					out_file = 'VNS-LNS-Hybrid-Solutions-Individual/VNS_LNS_' + file_name[:-4] + '_' + str(t) + '_g' + str(g) + '_' + str(w) +'.txt'
					run_code = './sdmi_LNS.out ' + inp_file + ' ' + cla  + ' ' + out_file
					...
					for iters in range(ITERATION_COUNT):
						...
						lines = []
						with open(path + 'VNS_LNS_' + file_name[:-4] + '_' + str(t) + '_g' + str(g) + '_' + str(w) +'.txt') as f:
							lines = f.readlines()
						if len(lines) < 8:
